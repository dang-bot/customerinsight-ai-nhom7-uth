...
from src.clustering_comparison import benchmark_all_algorithms, evaluate_kmeans_comprehensive
from src.bayes_classifier import train_bayes_classifier, predict_single_customer, BayesModelResult
...
def benchmark_page(request: Request):
    ...
    if session.app_state.processed_df is None:
        return _with_session(RedirectResponse(url="/eda", status_code=303), session_id)
    return _with_session(FileResponse(WEB / "templates" / "benchmark.html"), session_id)


@app.get("/predict", include_in_schema=False)
def predict_page(request: Request):
    session_id, session = _session(request)
    if session.app_state.cluster_profiles is None:
        return _with_session(RedirectResponse(url="/clustering", status_code=303), session_id)
    return _with_session(FileResponse(WEB / "templates" / "predict.html"), session_id)
...
def run_benchmark_endpoint(request: Request):
    session_id, session = _session(request)
    state = session.app_state
    if state.scaled_matrix is None:
        return _with_session(JSONResponse({"detail": "Cần tiền xử lý dữ liệu trước khi benchmark."}, status_code=422), session_id)

    target_k = int(state.selected_k) if state.selected_k is not None else 4
    try:
...
# Cache cho mô hình Bayes trong session/app
_bayes_cache: dict[str, Any] = {}

@app.post("/api/train-bayes")
def train_bayes_endpoint(request: Request):
    session_id, session = _session(request)
    state = session.app_state
    if state.results is None or state.cluster_profiles is None:
        return _with_session(JSONResponse({"detail": "Cần hoàn tất phân cụm trước khi huấn luyện Bayes."}, status_code=422), session_id)

    try:
        df_customers = state.results.copy()
        res = train_bayes_classifier(df_customers)
        _bayes_cache[session_id] = res
        return _with_session(_json_response({
            "status": "ok",
        ...
        return _with_session(JSONResponse({"detail": f"Lỗi huấn luyện Bayes: {exc}"}, status_code=500), session_id)


@app.post("/api/predict-customer")
async def predict_customer_endpoint(request: Request):
    session_id, session = _session(request)
    state = session.app_state

    # Tự động huấn luyện nếu chưa có trong cache
    if session_id not in _bayes_cache:
        if state.results is None:
            return _with_session(JSONResponse({"detail": "Chưa có mô hình. Vui lòng hoàn tất phân cụm và nhấn Huấn luyện Mô hình Bayes trước."}, status_code=422), session_id)
        _bayes_cache[session_id] = train_bayes_classifier(state.results)

    bayes_res = _bayes_cache[session_id]
    try:
        body = await request.json()
        recency = float(body.get("recency", 0))
        frequency = float(body.get("frequency", 1))
        monetary = float(body.get("monetary", 0))

        # Lấy tên phân khúc nếu có
        seg_names = {}
        if state.cluster_profiles is not None:
            for _, row in state.cluster_profiles.iterrows():
                seg_names[int(row["Cluster"])] = str(row["SegmentName"])

        pred = predict_single_customer(bayes_res.model, recency, frequency, monetary, seg_names)
        return _with_session(_json_response(pred), session_id)
    except Exception as exc:
        return _with_session(JSONResponse({"detail": f"Lỗi dự đoán: {exc}"}, status_code=400), session_id)
